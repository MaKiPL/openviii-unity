...
namespace OpenVIII
{
    ...
    public class TIM2 : Texture_Base
    {
        ...
        public override void SaveCLUT(string path)
        {
            ...
            using (var br = new BinaryReader(new MemoryStream(Buffer)))
            {
                var clut = new Texture2D(Texture.NumOfColors, Texture.NumOfCluts);
                for (ushort i = 0; i < Texture.NumOfCluts; i++)
                {
                    clut.SetPixels(0, i, Texture.NumOfColors, 1, GetClutColors(i));
                }
                using (var fs = new FileStream(path, FileMode.Create, FileAccess.ReadWrite, FileShare.ReadWrite))
                {
                    byte[] clutPng = clut.EncodeToPNG();
                    fs.Write(clutPng, 0, clutPng.Length);
                }
            }
        }
        ...
        public override void SavePNG(string path, short clut = -1)
        {
            ...
            using (var br = new BinaryReader(new MemoryStream(Buffer)))
            {
                var tex = GetTexture(br, !CLP ? null : GetClutColors(br, (ushort)clut));
                    if (tex != null)
                        ...
                        {
                        byte[] pngBuffer = tex.EncodeToPNG();
                        fs.Write(pngBuffer, 0, pngBuffer.Length);
                        }
            }
        }
