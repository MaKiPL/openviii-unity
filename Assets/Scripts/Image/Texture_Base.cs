...
namespace OpenVIII
{
    public abstract class Texture_Base
    {
        ...
        public static Color ABGR1555toRGBA32bit(ushort pixel, bool ignoreAlpha = false)
        {
            ...
            if (pixel == 0 && !ignoreAlpha) return Color.clear;
            ...
            var ret = new Color
            {
                r = (byte)Mathf.Clamp((pixel & red_mask) << 3, 0, 255),
                g = (byte)Mathf.Clamp(((pixel & green_mask) >> 5) << 3, 0, 255),
                b = (byte)Mathf.Clamp(((pixel & blue_mask) >> 10) << 3, 0, 255),
                a = 255
            };
            //if (GetSTP(pixel))
            //{
        ...
        public static Color Custom_16BITtoRGBA32bit(ushort pixel, ushort c_red_mask, ushort c_red_shift, ushort c_green_mask, ushort c_green_shift, ushort c_blue_mask, ushort c_blue_shift, bool ignoreAlpha = false)
        {
            ...
            var ret = new Color
            {
                r = (byte)Mathf.Clamp(((pixel & c_red_mask) >> c_red_shift) << 3, 0, 255),
                g = (byte)Mathf.Clamp(((pixel & c_green_mask) >> c_green_shift) << 3, 0, 255),
                b = (byte)Mathf.Clamp(((pixel & c_blue_mask) >> c_blue_shift) << 3, 0, 255),
                a = 255
            };
            return ret;
        }
