...
namespace OpenVIII
{
    ...
    public sealed class TEX : Texture_Base
    {
        ...
        {
            var clut = new Texture2D(_texture.NumOfColors, _texture.NumOfCluts);
                for (ushort i = 0; i < _texture.NumOfCluts; i++)
                {
                clut.SetPixels(0, i, _texture.NumOfColors, 1, GetClutColors(i));
                }
            using (var fs = new FileStream(path, FileMode.Create, FileAccess.ReadWrite, FileShare.ReadWrite))
            {
                byte[] pngBuffer = clut.EncodeToPNG();
